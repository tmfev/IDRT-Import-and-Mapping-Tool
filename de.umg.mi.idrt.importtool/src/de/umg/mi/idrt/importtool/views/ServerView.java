...
public class ServerView extends ViewPart {
	...
	//TODO
	public static void createMDRMenu() {
		if (mdrMenu!= null)
			mdrMenu.dispose();
		mdrMenu = new Menu(importMDRMenuItem);
		for (MenuItem m : mdrMenu.getItems()) {
			m.dispose();
		}
			...
			if (current.getDatabaseType().equalsIgnoreCase("oracle")) {
				...
				item.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent e) {
						MessageDialog.openInformation(Application.getShell(), "NYI", "NYI");
					}
				});
			}
		}
