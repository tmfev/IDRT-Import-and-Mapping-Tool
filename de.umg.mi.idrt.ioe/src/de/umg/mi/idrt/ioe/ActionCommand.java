...
public class ActionCommand {
	...
	public ActionCommand(ExecutionEvent event){
		...
		while (parametersIterator.hasNext()){
			...
			_params.put(parameterKey, checkParameter(parameters.get(parameterKey)));
		}
	}
	
	public void addParameter(String id, Object value){
		System.out.println("addingParamter " + id + ":" + String.valueOf( value ) );
	...
	public ParameterizedCommand getParameterizedCommand(){
		...
		if (activeWorkbenchWindow != null) {
			...
			if (commandService != null){
				ParameterizedCommand parameterizedCommand = ParameterizedCommand.generateCommand(commandService.getCommand(_commandID), _params);
	...
	public ParameterizedCommand getCommand(){
		...
		if (activeWorkbenchWindow != null) {
			...
			if (commandService != null){
				...
				Debug.dn("commandService.getCommand(_commandID)",commandService.getCommand(_commandID));
				Debug.dn("_params",_params);
				
				//commandService.
				Debug.dn("parameterizedCommand",ParameterizedCommand.generateCommand(commandService.getCommand(_commandID), null));
				ParameterizedCommand parameterizedCommand = ParameterizedCommand.generateCommand(commandService.getCommand(_commandID), _params);
				return parameterizedCommand;
			}
		}
	...
	public Boolean getBoolean(String key){
		...
		if (_params.get(key).getClass().equals(Boolean.class)){
			...
			Debug.d("A boolean parameter wasn't really a boolean but a " + _params.get(key).getClass().getName() + ". Trying to convert to now ...");
