...
//Simport swingintegration.example.EmbeddedSwingComposite;
...
public class EditorTargetView extends ViewPart {
	...
	*/
	...
	public void setComposite(OntologyTree tree){
		...
		treeContentProvider.setOT( this._i2b2ImportTool.getMyOntologyTrees().getOntologyTreeTarget());
		//treeContentProvider.setViewTree( this._i2b2ImportTool.getMyOntologyTrees().getViewTreeTarget() );
		...
		//ContentProviderTree contentProviderTree = new ContentProviderTree( _i2b2ImportTool.getMyOT().getOT() );
		//viewer.setInput(ContentProviderTree.INSTANCE.getModel());
		_treeViewer.setInput(new OTtoTreeContentProvider().getModel());

		_treeViewer.expandToLevel(8);

		final TreeViewer viewer2 = _treeViewer;
				
		this._i2b2ImportTool.getMyOntologyTrees().getOntologyTreeTarget().setTreeViewer(_treeViewer);
		...
		menu.addMenuListener(new MenuAdapter() {
	         public void menuShown(MenuEvent e) {
	            ...
	            */
	            ...
	            //newItem.setText("Menu for " + viewer2.getTree().getSelection()[0].getText());
	         }
	      });
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (node != null){
		    		...
		    		if (!node.getNodeType().equals( NodeType.ITEM )){
						...
						//
						//Activator.getDefault().getResource().getOntologyAnswersPreviewView().setItemNode(null);
					}
					*/

					Activator.getDefault().getResource().getEditorTargetInfoView().setNode(node);
		
		    		Application.getStatusView().addMessage(new SystemMessage ("Target selection changed to \'" + node.getName() + "\'.",
							SystemMessage.MessageType.SUCCESS,
							SystemMessage.MessageLocation.MAIN));
				} else {
		    		Application.getStatusView().addMessage(new SystemMessage ("Target selection changed but new selection isnt' any know kind of node..",
	...
	public Menu getContextMenu(Composite composite){
	    ...
	    mntmInsert.setImage(ResourceManager.getPluginImage("edu.goettingen.i2b2.importtool", "images/edit-copy.png"));
	    mntmInsert.setText("Insert");
	    
	    MenuItem mntmCombine = new MenuItem(menu, SWT.PUSH);
	    mntmCombine.setImage(ResourceManager.getPluginImage("edu.goettingen.i2b2.importtool", "images/format-indent-more.png"));
